...
int next_unvisted(int num_vertices, bool* visited, result* res)
{
  int next = -1;
...
result* dijkstra(int** graph, int num_vertices, int source)
{
  ...
  int u = -1;
  while((u = next_unvisted(num_vertices, visited, res)) != -1)
  {
    //std::cout << "\nvisiting " << u << std::endl;
    visited[u] = true;
...
void print_shortest_path(result* res, int source, int dest)
{
...
void print_result(result* res, int num_vertices, int source)
{
  ...
  {
    print_shortest_path(res, source, dest);
  }
}
